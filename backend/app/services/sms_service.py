...
class SMSService:
    ...
    def __init__(self):
        ...
        if settings.TWILIO_ACCOUNT_SID and settings.TWILIO_AUTH_TOKEN and settings.TWILIO_PHONE_NUMBER:
            ...
            logger.warning("Twilio credentials are not configured. SMS delivery will be logged only.")
    
    def send_emergency_alert(self, to_numbers: List[str], user_data: Dict, location: Dict, message_override: Optional[str] = None) -> bool:
        """Send emergency alert SMS to contacts"""
        try:
            message = message_override or self._format_emergency_message(user_data, location)

            if not self.client:
                for number in to_numbers:
                    logger.info("Would send emergency alert to %s: %s", number, message)
                return True
