...
class _EmergencyScreenState extends State<EmergencyScreen>
  ...
  Future<void> _sendSOS(BuildContext context) async {
    ...
    try {
      ...
      final userId = prefs.getString(AppConstants.PREF_USER_ID) ?? 'anonymous';
      final userName = prefs.getString(AppConstants.PREF_USER_NAME) ?? 'User';

      await emergencyService.triggerOfflineEmergency(
        userId: userId,
      ...
      if (mounted) {
        setState(() {
          _sosStatus = '✅ SOS Alert Sent Successfully!';
          _emergencyCount++;
          _voiceStatus = 'Emergency mode activated';
        });
      }
    } catch (e) {
      if (mounted) {
